...
void AodvExample::closeLogStream ()  //---------
{
		oss.close ();
}
...
//  AnimationInterface anim ("MP_trace.xml");
...
AodvExample::InstallInternetStack ()
{
  ...
  if (printRoutes)
    {
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("RDPZ_aodv.routes", std::ios::out);
      for (uint32_t i = 0; i < totalTime/100; i++)
      {
      aodv.PrintRoutingTableAllAt (Seconds (i*100), routingStream);
      }
    }
}
